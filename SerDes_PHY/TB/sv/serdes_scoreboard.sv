...
class serdes_scoreboard extends uvm_scoreboard;
    ...
    function void write_exp(serdes_packet packet);
        ...
        bit [31:0] value;
        packets_in++;
        // p = serdes_packet::type_id::create("p");
        $cast(scb_packet, packet.clone());
        value = scb_packet.get_payload_for_dir(scb_packet.dir);
        exp_queue.push_back(scb_packet);
        `uvm_info(get_type_name(), $sformatf("Expected packet queued: dir=%s payload=%0h", scb_packet.dir.name(), value), UVM_HIGH)
    endfunction
    ...
    function void write_mon(serdes_packet packet);
        ...
        `uvm_info(get_type_name(), $sformatf("Actual packet queued: dir=%s payload=%0h", scb_packet.dir.name(), value), UVM_HIGH)
    endfunction
    ...
    function void check_phase(uvm_phase phase);
        ...
        // So sánh theo thứ tự cho đến khi một trong hai queue rỗng
            ...
            case (compare_policy)
                EQUALITY: begin
                    if (exp_pkt.dir == act_pkt.dir &&
                        exp_pkt.get_payload_for_dir(exp_pkt.dir) == act_pkt.get_payload_for_dir(act_pkt.dir)) begin
                        pass_count++;
                        `uvm_info(get_type_name(),
                            $sformatf("MATCH: dir=%s payload=%0h", act_pkt.dir.name(),
                                    act_pkt.get_payload_for_dir(act_pkt.dir)), UVM_MEDIUM)
                    end else begin
                        fail_count++;
                        `uvm_error(get_type_name(),
                            $sformatf("MISCOMPARE: exp dir=%s payload=%0h, act dir=%s payload=%0h",
                                    exp_pkt.dir.name(), exp_pkt.get_payload_for_dir(exp_pkt.dir),
                                    act_pkt.dir.name(), act_pkt.get_payload_for_dir(act_pkt.dir)))
                    end
                end
                ...
                UVM: begin
                    if (exp_pkt.compare(act_pkt)) begin
                        pass_count++;
                        `uvm_info(get_type_name(), "MATCH via uvm_object::compare()", UVM_MEDIUM)
                    end else begin
                        fail_count++;
                        `uvm_error(get_type_name(), $sformatf("MISCOMPARE via uvm_object::compare()\nEXP:\n%s\nACT:\n%s",
        ...
        // Nếu còn dư expected hoặc actual
    ...
    function void report_phase(uvm_phase phase);
        `uvm_info(get_type_name(), $sformatf("Scoreboard summary: packets_in=%0d packets_out=%0d pass=%0d fail=%0d", packets_in, packets_out, pass_count, fail_count), UVM_LOW)
